...
OUTPUT_PDF_2024 <- if (basename(script_dir) == "visualization") {
  file.path(dirname(dirname(script_dir)), "outputs", "plots", "2024_papers_vs_committee_continent_gap.pdf")
} else {
  file.path(script_dir, "outputs", "plots", "2024_papers_vs_committee_continent_gap.pdf")
}
...
process_heatmap_data_2024 <- function(papers_path, committee_path) {
  papers <- read.csv(papers_path, stringsAsFactors = FALSE, na.strings = "")
  committee <- read.csv(committee_path, stringsAsFactors = FALSE, na.strings = "")
...
dir.create(dirname(OUTPUT_PDF_2024), recursive = TRUE, showWarnings = FALSE)
ggsave(OUTPUT_PDF_2024, plot = plot, device = "pdf", width = 9.5, height = pdf_height, units = "in")
